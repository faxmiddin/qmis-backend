...
console.log('[PROXY] Pool:', POOL_HOST + ':' + POOL_PORT);
...
wss.on('connection', (ws) => {
  ...
  tcp.connect(POOL_PORT, POOL_HOST, () => {
    clearTimeout(timer);
    console.log('[PROXY] ✅ TCP connected to pool!');

    // Subscribe
    sendPool({
      id: 1,
      method: 'mining.subscribe',
      params: ['cgminer/4.10.0', null, POOL_HOST, POOL_PORT]
    });
  ...
  tcp.on('data', (chunk) => {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (msg.id === 2) {
          if (msg.result === true) {
            console.log('[PROXY] ✅✅✅ AUTHORIZED! Real mining active!');
          } else {
